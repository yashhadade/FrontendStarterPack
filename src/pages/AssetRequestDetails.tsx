...
const stepIndex = (status: string) => {
  ...
  if (
    normalized === 'ASSET_CREATION_PROCESSING' ||
    normalized === 'COMPLETED' ||
    normalized === 'ASSET_CREATED'
  )
    return 2;
...
const statusBadgeClass = (status: TransferInvestor['status']) => {
  switch (status) {
    ...
    case 'TOKEN_TRANSFERRED_INITIATED':
      return 'bg-yellow-100 text-yellow-700 border border-yellow-200';
    case 'TOKEN_TRANSFERRED':
      return 'bg-emerald-100 text-emerald-700 border border-emerald-200';
    case 'REJECTED':
      return 'bg-red-100 text-red-700 border border-red-200';
    case 'TOKEN_TRANSFERRED_FAILED':
      return 'bg-red-100 text-red-700 border border-red-200';
  }
};
...
const statusLabel = (status: TransferInvestor['status']) => {
  switch (status) {
    ...
    case 'TOKEN_TRANSFERRED_FAILED':
      return 'Transfer Cancelled. You can initiate again';
  }
};
...
const AssetRequestDetails = () => {
  ...
  const [transferTab, setTransferTab] = useState<
    'APPROVED' | 'TOKEN_TRANSFERRED_INITIATED' | 'TOKEN_TRANSFERRED'
  >('APPROVED');
          ...
          <div className="glass-card p-6 space-y-5">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Client Submitted — Awaiting Custodian Review
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                { icon: User, label: 'Seller Name', value: asset?.sellerName ?? '-' },
                {
                  icon: Building2,
                  label: 'Asset Name',
                  value: asset.assetName ?? `Asset #${asset._id}`,
                },
                {
                  icon: CreditCard,
                  label: 'Total Asset Value',
                  value:
                    asset?.totalAssetValueInInr != null
                      ? `₹${Number(asset.totalAssetValueInInr).toLocaleString('en-IN')}`
                      : '-',
                },
                // {
                //   icon: Hash,
                ...
                // { icon: FileText, label: "Ownership Contract", value: asset.ownershipContractId ?? "REG/MH/2024/001234" },
                {
                  ...
                  value: (asset?.noOfTokens ?? '-') + ' ' + (asset?.tokenName ?? '-'),
                },
              ...
              <div className="mt-3 space-y-1">
                ...
                <div className="relative inline-flex items-center">
                  ...
                  <button
                    ...
                    aria-label={
                      showExistingIpfsPassword ? 'Hide IPFS password' : 'Show IPFS password'
                    }
                  >
                    {showExistingIpfsPassword ? (
          ...
          <div className="bg-card border border-border rounded-2xl shadow-xl p-6 w-full max-w-md space-y-4 animate-fade-in">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  This password will be securely sent to the backend as part of the approval
                  process.
                </p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/20 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl shadow-xl p-6 w-full max-w-md space-y-4 animate-fade-in">
            ...
            <div className="space-y-2">
              ...
              />
              {rejectError && <p className="text-[11px] text-destructive mt-1">{rejectError}</p>}
            </div>

...
export const MintAndTransferSection = ({
  ...
  const [signingOfTheLegalNote, setSigningOfTheLegalNote] = useState(
    asset?.signedLegalNote || false
  );
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-start gap-3 p-4 rounded-xl bg-muted/40 border border-border">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
              Transfers are processed in batches to reduce gas and improve reliability. Select up to
              80 investors per transaction; if more are pending, run multiple batches.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between p-4 rounded-xl bg-card border border-border shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            {transferTab === 'APPROVED' && (
              <button
                onClick={() => selectedRows.size > 0 && setShowGasModal(true)}
                disabled={selectedRows.size === 0 || !asset?.isTokenMinted}
                className={`px-5 h-8 rounded-lg text-sm font-semibold text-white transition-colors shadow-sm ${
                  selectedRows.size > 0
                ...
                <th className="text-left py-3 px-4 w-10">
                  <Checkbox
                    checked={
                      ...
                      selectedRows.size === Math.min(80, filteredInvestors.length)
                    }
                ...
                filteredInvestors?.map((inv) => (
                  <tr
                    ...
                    className={`border-b border-border/30 transition-colors cursor-pointer ${
                      selectedRows.has(inv._id) ? 'bg-accent/40' : 'hover:bg-muted/30'
                    }`}

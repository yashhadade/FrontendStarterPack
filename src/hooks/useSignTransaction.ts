...
const useSignTransaction = () => {
  const signTransaction = async (
    ...
    try {
      ...
      const isOwner = tx?.owners?.some(
        (owner) => owner.toLowerCase() === publicAddress.toLowerCase()
      );
      if (!isOwner) {
      ...
      if (getTransactionToSignResponse.data && !getTransactionToSignResponse.data.success) {
        toast.error(
          getTransactionToSignResponse.data.message || 'Failed to get transaction to sign'
        );
        return { success: false };
      }
